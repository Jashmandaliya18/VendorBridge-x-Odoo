...
const VendorsPage = () => {
  ...
  const handleStatusConfirm = () => {
    if (vendorToChange) {
      const nextStatus = vendorToChange.status === 'blocked' ? 'active' : 'blocked';
      statusMutation.mutate({ id: vendorToChange._id, status: nextStatus });
    }
  };
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center space-x-2.5">
          <Link to={`/vendors/${row._id}`} className="text-slate-500 hover:text-slate-700 p-1 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors">
          ...
          <button 
            onClick={() => { setVendorToChange(row); setConfirmStatusOpen(true); }}
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <ConfirmDialog
        ...
        title={vendorToChange?.status === 'blocked' ? "Activate Vendor" : "Block Vendor"}
        message={`Are you sure you want to ${vendorToChange?.status === 'blocked' ? 'activate' : 'block'} ${vendorToChange?.name}?`}
        type={vendorToChange?.status === 'blocked' ? 'teal' : 'danger'}
        confirmLabel={vendorToChange?.status === 'blocked' ? "Activate Partner" : "Block Partner"}
      />
    </div>
  );
};
